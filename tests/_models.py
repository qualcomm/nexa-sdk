...
model family / size / instruction-tuning. AI Hub publishes nothing smaller
than Qwen3-4B (LLM) and Qwen2.5-VL-7B (VLM) in the Qwen family, so the
GGUF side is lifted up to match rather than QAIRT being scaled down. With
the cells aligned, a keyword-quality divergence between the two plugins
traces to backend / quantization rather than to model identity.
...
LLAMA_CPP_LLM_MODEL = 'unsloth/Qwen3-4B-Instruct-2507-GGUF'
