...
import com.geniex.demo.FileConfig
...
fun ModelData.getGeniexManifest(context: Context): GeniexManifestBean? {
    try {
        val str = File(modelDir(context), "geniex.json").bufferedReader().use { it.readText() }
        return Json {
            ignoreUnknownKeys = true
        }.decodeFromString<GeniexManifestBean>(str)
    } catch (e: Exception) {
        e.printStackTrace()
        return null
    }
