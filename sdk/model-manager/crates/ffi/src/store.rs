...
/// C-compatible model type enum (mirrors ml_ModelType in ml_model.h).
#[repr(C)]
...
fn to_ffi_type(t: ModelType) -> MlModelType {
    match t {
        ModelType::Llm => MlModelType::Llm,
        ModelType::Vlm => MlModelType::Vlm,
        ModelType::Embedder => MlModelType::Embedder,
        ModelType::Reranker => MlModelType::Reranker,
        ModelType::Tts => MlModelType::Tts,
        ModelType::Asr => MlModelType::Asr,
        ModelType::Diarize => MlModelType::Diarize,
        ModelType::Cv => MlModelType::Cv,
        ModelType::ImageGen => MlModelType::ImageGen,
    }
}
...
#[no_mangle]
pub extern "C" fn ml_model_get_paths(
    model_name: *const c_char,
    out_paths: *mut MlModelPaths,
) -> i32 {
    ...
    match store.get_paths(name) {
        Ok((_, paths)) => {
            unsafe {
                ...
                (*out_paths).mmproj_path = paths.mmproj_path
                    .as_ref()
                    .map(|p| str_to_cptr(&p.to_string_lossy()))
                    .unwrap_or(std::ptr::null_mut());
                (*out_paths).tokenizer_path = paths.tokenizer_path
                    .as_ref()
                    .map(|p| str_to_cptr(&p.to_string_lossy()))
                    .unwrap_or(std::ptr::null_mut());
                (*out_paths).device_id = paths.device_id
                    .as_deref()
                    .map(str_to_cptr)
                    .unwrap_or(std::ptr::null_mut());
            }
...
#[no_mangle]
pub unsafe extern "C" fn ml_model_paths_free(paths: *mut MlModelPaths) {
    if paths.is_null() {
        return;
    }
    let p = &mut *paths;
    ...
    *paths = MlModelPaths::null();
}

/* ---- ml_model_list ---- */

#[repr(C)]
pub struct MlModelListOutput {
    pub names: *mut *mut c_char,
    pub count: i32,
}

#[no_mangle]
pub extern "C" fn ml_model_list(output: *mut MlModelListOutput) -> i32 {
    if output.is_null() {
        return ML_ERROR_COMMON_INVALID_INPUT;
    }
    let store = match get_store() {
    ...
    match store.list() {
        Ok(manifests) => {
            let mut ptrs: Vec<*mut c_char> = manifests
                .iter()
                .map(|m| str_to_cptr(&m.name))
                .collect();
            ptrs.shrink_to_fit();
...
#[no_mangle]
pub unsafe extern "C" fn ml_model_list_free(output: *mut MlModelListOutput) {
    if output.is_null() {
        return;
    }
    let o = &mut *output;
    if !o.names.is_null() && o.count > 0 {
        let slice = std::slice::from_raw_parts_mut(o.names, o.count as usize);
        for ptr in slice.iter_mut() {
            free_cptr(*ptr);
        }
        // Reclaim the Vec<*mut c_char> allocation.
        drop(Vec::from_raw_parts(o.names, o.count as usize, o.count as usize));
    }
    o.names = std::ptr::null_mut();
    o.count = 0;
}

/* ---- ml_model_remove / clean ---- */

#[no_mangle]
pub extern "C" fn ml_model_remove(model_name: *const c_char) -> i32 {
    let name = match unsafe { cstr_to_str(model_name) } {
        Some(s) => s,
        None => return ML_ERROR_COMMON_INVALID_INPUT,
    };
...
#[no_mangle]
pub extern "C" fn ml_model_clean(removed_count: *mut i32) -> i32 {
    let store = match get_store() {
        Ok(s) => s,
        Err(c) => return c,
    };
...
#[no_mangle]
pub extern "C" fn ml_model_get_type(
    model_name: *const c_char,
    out_type: *mut MlModelType,
) -> i32 {
    if out_type.is_null() {
        return ML_ERROR_COMMON_INVALID_INPUT;
    }
