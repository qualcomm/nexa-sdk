...
pub struct ModelPaths {
    ...
    /// For ImageGen: path to the model directory instead of a file.
...
pub fn resolve_model_paths(
    ...
    let (resolved_quant, model_path) = match manifest.model_type {
        ModelType::ImageGen => {
            // ImageGen uses the directory, not a specific file.
            let q = quant.unwrap_or("").to_string();
            (q, model_dir.clone())
        }
        _ => {
