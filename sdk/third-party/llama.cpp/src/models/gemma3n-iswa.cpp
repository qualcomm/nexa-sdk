...
ggml_tensor * llm_build_gemma3n_iswa::get_per_layer_inputs() {
    ...
    if (ubatch.token) {
        ...
        // For multimodal: support embd input by returning nullptr
        // The projection will handle the nullptr case
        inp_per_layer = nullptr;
    }
...
ggml_tensor * llm_build_gemma3n_iswa::project_per_layer_inputs(ggml_tensor * inputs_embeds, ggml_tensor * inp_per_layer) {
    ...
    // For multimodal: handle the case when inp_per_layer is nullptr (embd input)
    if (inp_per_layer == nullptr) {
        per_layer_proj = ggml_cont(ctx0, ggml_permute(ctx0, per_layer_proj, 0, 2, 1, 3));
