...
std::vector<float> load_wav_as_float32(const std::string &file_path) {
  ...
  GENIEX_LOG_INFO("WAV file info - Sample rate: {}Hz, Channels: {}, Bits: {}, "
                "Data size: {} bytes",
...
void streaming_transcription_callback(const char *text, void *user_data) {
  ...
  if (text && strlen(text) > 0) {
    ...
    test_data->accumulated_text =
        text; // Keep the latest complete transcription
  }
}
...
void test_asr_transcribe_basic(ml_ASR *asr, const std::string &model_name) {
  ...
  CHECK(utf8::is_valid(output.result.transcript,
                       output.result.transcript +
                           std::strlen(output.result.transcript)));

  std::string transcript_lower(output.result.transcript);
  std::transform(transcript_lower.begin(), transcript_lower.end(),
                 transcript_lower.begin(), ::tolower);
  ...
  for (const auto &keyword : expected_keywords) {
    ...
    auto error_msg =
        fmt::format("Expected keyword '{}' not found in transcript: '{}'",
                    keyword, output.result.transcript);
  ...
  if (output.result.confidence_count > 0) {
    ...
    int max_print =
        std::min(5, static_cast<int>(output.result.confidence_count));
  ...
  if (output.result.timestamp_count > 0) {
    ...
    int max_print =
        std::min(3, static_cast<int>(output.result.timestamp_count));
    for (int i = 0; i < max_print; i++) {
...
void test_asr_list_supported_languages(ml_ASR *asr,
  ...
  if (output.language_count > 0) {
    ...
    for (int32_t i = 0; i < max_print; i++) {
      ...
      CHECK(utf8::is_valid(output.language_codes[i],
                           output.language_codes[i] +
                               std::strlen(output.language_codes[i])));
    }
...
void test_asr_streaming_transcription(ml_ASR *asr,
  ...
  if (model_name.find("parakeet") == std::string::npos) {
    GENIEX_LOG_INFO("Skipping streaming test for model '{}' - only parakeet "
  ...
  GENIEX_LOG_INFO("Loaded {} audio samples for streaming test",
                audio_data.size());
  ...
  GENIEX_LOG_INFO("Starting to stream {} total samples in chunks of {}",
  ...
  GENIEX_LOG_INFO("Final streaming transcription: '{}'",
                test_data.accumulated_text);
  GENIEX_LOG_INFO("Received {} streaming updates",
                test_data.streaming_updates.size());

  // Print all streaming updates
  for (size_t i = 0; i < test_data.streaming_updates.size(); i++) {
    GENIEX_LOG_INFO("Streaming update {}: '{}'", i + 1,
                  test_data.streaming_updates[i]);
  }

  // Validate UTF-8 encoding of the final transcription
  CHECK(utf8::is_valid(test_data.accumulated_text.c_str(),
                       test_data.accumulated_text.c_str() +
                           test_data.accumulated_text.length()));
  ...
  for (const auto &keyword : expected_keywords) {
    ...
    auto error_msg = fmt::format(
        "Expected keyword '{}' not found in streaming transcript: '{}'",
        keyword, test_data.accumulated_text);
    CHECK_MESSAGE(found, error_msg.c_str());
    if (found) {
      GENIEX_LOG_INFO("Found expected keyword '{}' in streaming transcript",
                    keyword);
    } else {
      GENIEX_LOG_WARN("Missing expected keyword: '{}'", keyword);
    }
...
void register_asr_tests(TestRegistry<ml_ASR> &registry) {
  ...
  if (std::is_same_v<PluginType, qnn>) {
    ...
    REGISTER_TEST(registry, AsrStreamingTranscription,
                  test_asr_streaming_transcription(model, model_name););
  }
}
