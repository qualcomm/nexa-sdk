  ...
  if (output.result_count > 0) {
    ...
    for (int i = 0; i < output.result_count; i++) {
      ...
      if (cv_result.text != nullptr) {
        ...
        CHECK(utf8::is_valid(cv_result.text,
                             cv_result.text + std::strlen(cv_result.text)));
      }
      ...
      if (cv_result.bbox.width > 0 && cv_result.bbox.height > 0) {
        GENIEX_LOG_INFO("  Bounding Box: [{}, {}, {}, {}]", cv_result.bbox.x,
                      cv_result.bbox.y, cv_result.bbox.width,
