...
Setup<Param, geniex_Embedder> setup_guard(
    SetupMap<Param>{
        {llama_cpp::value,
         std::vector<Param>{
             {"jina-v2-small-Q4", "jina-v2-small-Q4",
...
// QAIRT tests are not supported on this platform (Linux/other)
    ...
    [](geniex_PluginId plugin, Param param) {
      ...
      if (mmproj_path.has_value() &&
          !std::filesystem::exists(mmproj_path.value())) {
      ...
      input.mmproj_path =
          mmproj_path.has_value() ? mmproj_path.value().c_str() : nullptr;
...
void test_embedder_single_text(geniex_Embedder *embedder,
  ...
  GENIEX_LOG_INFO("Generated {} embeddings with dimension {}",
                output.embedding_count, dim_output.dimension);
  ...
  if (output.embeddings != nullptr && expected_total_floats > 0) {
    ...
    float mean =
        std::accumulate(output.embeddings,
                        output.embeddings + expected_total_floats, 0.0f) /
        expected_total_floats;
    float variance = 0.0f;
    ...
    GENIEX_LOG_INFO("Embedding stats: min={}, max={}, mean={}, std={}",
                  *std::min_element(output.embeddings,
                                    output.embeddings + expected_total_floats),
                  *std::max_element(output.embeddings,
                                    output.embeddings + expected_total_floats),
                  mean, std_dev);
  }
...
void test_embedder_batch_processing(geniex_Embedder *embedder,
  ...
  std::vector<std::string> text_strs = {"Hello world", "Good morning",
...
void test_image_search(geniex_Embedder *embedder, const std::string &model_name) {
  ...
  auto manhattan_distance = [](const float *vec_a, const float *vec_b,
                               int dim) {
    float distance = 0.0f;
    for (int i = 0; i < dim; ++i) {
  ...
  float dist_query_blue = manhattan_distance(text_output.embeddings,
                                             blue_cat_output.embeddings, dim);
  float dist_query_red = manhattan_distance(text_output.embeddings,
                                            red_cat_output.embeddings, dim);

  GENIEX_LOG_INFO("Manhattan distance (query 'A blue cat' -> blue_cat.jpg): {}",
                dist_query_blue);
  GENIEX_LOG_INFO("Manhattan distance (query 'A blue cat' -> red_cat.jpg): {}",
                dist_query_red);
...
void register_embedder_tests(TestRegistry<geniex_Embedder> &registry) {
  ...
  REGISTER_TEST(registry, EmbedderSingleText,
                test_embedder_single_text(model, model_name););
  REGISTER_TEST(registry, EmbedderSemanticSimilarity,
                test_embedder_semantic_similarity(model, model_name););
  REGISTER_TEST(registry, EmbedderBatchProcessing,
                test_embedder_batch_processing(model, model_name););
}
