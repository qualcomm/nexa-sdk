...
void test_tts_synthesize_basic(geniex_TTS *tts, const std::string &model_name) {
  ...
  CHECK(utf8::is_valid(output.result.audio_path,
                       output.result.audio_path +
                           std::strlen(output.result.audio_path)));
  ...
  CHECK(output.result.duration_seconds <=
        10.0f); // Should not be too long for test text
...
void test_tts_list_available_voices(geniex_TTS *tts,
  ...
  if (output.voice_count > 0) {
    ...
    for (int32_t i = 0; i < max_print; i++) {
      ...
      CHECK(utf8::is_valid(output.voice_ids[i],
                           output.voice_ids[i] +
                               std::strlen(output.voice_ids[i])));
    }

    // Check that common voices are available
    std::vector<std::string> expected_voices = {"af_heart", "af_bella",
                                                "am_adam", "am_michael"};
    for (const auto &expected_voice : expected_voices) {
      ...
      if (!found) {
        GENIEX_LOG_WARN("Expected voice '{}' not found in available voices",
                      expected_voice);
      }
    }
...
void register_tts_tests(TestRegistry<geniex_TTS> &registry) {
  ...
  REGISTER_TEST(registry, TtsListAvailableVoices,
                test_tts_list_available_voices(model, model_name););
  REGISTER_TEST(registry, TtsErrorHandling,
                test_tts_error_handling(model, model_name););
}
