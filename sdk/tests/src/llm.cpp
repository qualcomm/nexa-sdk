...
struct PluginCapabilities {
  ...
  static bool has_kv_cache(const std::string &plugin) {
    return KV_CACHE.count(plugin) > 0;
  }
  static bool has_json(const std::string &plugin) {
    return JSON.count(plugin) > 0;
  }
  static bool has_tool_call(const std::string &plugin) {
    return TOOL_CALL.count(plugin) > 0;
  }
};
        ...
        {llama_cpp::value,
         {
             ...
             {"granite-4.0-h-350m-Q8_0", "granite-4.0-h-350m-Q8_0",
              "/data/local/tmp/geniex/modelfiles/llama_cpp/"
              "granite-4.0-h-350m-Q8_0.gguf",
              std::nullopt},
             {"Llama-3.2-3B-Instruct-Q4_0", "Llama-3.2-3B-Instruct-Q4_0",
              "/data/local/tmp/geniex/modelfiles/llama_cpp/"
              "Llama-3.2-3B-Instruct-Q4_0.gguf",
              std::nullopt},
             {"Qwen3-4B-Q4_0", "Qwen3-4B-Q4_0",
        ...
        {qairt::value,
         {
             ...
             {"Granite-4-Micro-NPU", "granite4",
              ...
              "weights-1-3.nexa",
              std::nullopt},
             {"Granite-4.0-h-350M-NPU", "granite4-nano",
             ...
             {"phi3.5-mini-npu", "phi3.5",
              ...
              "weights-1-3.nexa",
              std::nullopt},
             {"Qwen3-4B-Instruct-2507-NPU", "qwen3-4b",
              "/data/local/tmp/geniex/modelfiles/Qwen3-4B-Instruct-2507-NPU/"
              "weights-1-3.nexa",
              std::nullopt},
#elif defined(_WIN32)
             {"granite4_micro", "granite4",
              "modelfiles/qairt/granite4_micro/tokenizer.json", std::nullopt},
             {"phi4", "phi4", "modelfiles/qairt/phi4/tokenizer.json",
              std::nullopt},
             {"phi3_5_aihub", "phi3.5-aihub",
              "modelfiles/qairt/phi3_5_aihub/tokenizer.json", std::nullopt},
             {"qwen3_4b_aihub", "qwen3-4b-aihub",
              "modelfiles/qairt/qwen3_4b_aihub/tokenizer.json", std::nullopt},
             {"qwen3_4b_instruct_2507_aihub", "qwen3-4b",
...
void validate_and_reset_utf8_state(
  ...
  if (g_invalid_tokens_count > 0) {
    ...
    bool acceptable_invalid_utf8 =
        (g_invalid_tokens_count == 1) && g_last_token_was_invalid_utf8;

    if (!acceptable_invalid_utf8) {
      GENIEX_LOG_ERROR("Found {} invalid UTF-8 token(s) out of {} total tokens",
                     g_invalid_tokens_count, g_total_tokens_checked);
      GENIEX_LOG_ERROR(
          "Invalid UTF-8 found in non-final tokens - this indicates a problem");
      CHECK(false);
    } else {
      GENIEX_LOG_INFO("Allowing invalid UTF-8 in last token (incomplete sequence "
                    "at generation end)");
    }
  }
...
void register_llm_tests(TestRegistry<geniex_LLM> &registry) {
  ...
  REGISTER_TEST(
      ...
      REQUIRE(output.full_text != nullptr); CHECK(utf8::is_valid(
          output.full_text, output.full_text + std::strlen(output.full_text)));
  ...
  REGISTER_TEST(
      ...
      CHECK(utf8::is_valid(output.full_text,
                           output.full_text + std::strlen(output.full_text)));
  ...
  REGISTER_TEST(
      registry, GenerateChatMultiRound,
      struct Rounds { std::string user_message; };
      ...
      for (size_t i = 0; i < turns.size(); ++i) {
        ...
        if (apply_output.formatted_text)
          free(apply_output.formatted_text);
      }
