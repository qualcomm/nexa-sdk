...
// Helper function to detect Intel Vulkan devices (skip Vulkan for Intel)
static bool is_intel_vulkan_device(ggml_backend_dev_t device) {
    ...
    std::string desc     = dev_desc ? dev_desc : "";

    bool has_intel  = (name.find("Intel") != std::string::npos || desc.find("Intel") != std::string::npos);
    bool has_vulkan = (name.find("Vulkan") != std::string::npos || desc.find("Vulkan") != std::string::npos ||
                       name.find("vulkan") != std::string::npos || desc.find("vulkan") != std::string::npos);

    return has_intel && has_vulkan;
}

// Helper function to get model block count from GGUF metadata without loading full model
static int32_t get_model_block_count(const char* model_path) {
    ...
    int arch_key_id = gguf_find_key(ctx_gguf, "general.architecture");
    if (arch_key_id < 0) {
        gguf_free(ctx_gguf);
        return -1;
    }
    const char* arch_name = gguf_get_val_str(ctx_gguf, arch_key_id);
    if (!arch_name) {
        gguf_free(ctx_gguf);
        return -1;
    }

    // Build block_count key: "{arch}.block_count"
    std::string block_count_key    = std::string(arch_name) + ".block_count";
    int         block_count_key_id = gguf_find_key(ctx_gguf, block_count_key.c_str());
    if (block_count_key_id < 0) {
        gguf_free(ctx_gguf);
        return -1;
    }

    uint32_t block_count = gguf_get_val_u32(ctx_gguf, block_count_key_id);
    gguf_free(ctx_gguf);

    return static_cast<int32_t>(block_count);
}
...
int32_t LlamaLlm::create_impl(const ml_LlmCreateInput* input) {
    ...
    // Initialize instance-level tensor buffer override array for MoE expert tensors
    // Force specific MoE expert tensors to run on CPU instead of HTP NPU
    // These patterns match: blk.{layer}.ffn_{gate|up|down}_exps.{weight|bias}
    // Only apply this override for GPT OSS models, because mxfp4 data type is not supported in GGML Hexagon
    if (is_gpt_oss_model) {
        // Regex pattern to match all MoE expert tensors
        // This includes: ffn_gate_exps, ffn_up_exps, ffn_down_exps (both .weight and .bias)
        ...
        GENIEX_LOG_INFO("GPT OSS model detected - MoE expert tensors (ffn_*_exps.weight/bias) will be forced to CPU");
    } else {
    ...
    if (input->device_id) {
        ...
        if (!devices.empty()) {
            // Copy to instance-level array and null-terminate
            for (size_t i = 0; i < devices.size() && i < 8; ++i) {
                this->device_array[i] = devices[i];
                if (i == 0) {
                }
            }
            this->device_array[devices.size()] = nullptr;
            mpar.devices                       = this->device_array;
        ...
        // Follow llama.cpp's default device selection priority:
        // 1. GPU (discrete GPUs) - highest priority
        // 2. IGPU (integrated GPUs) - only if no GPUs found
        // This ensures we check the device that will actually be used
        ggml_backend_dev_t first_gpu  = nullptr;
        ggml_backend_dev_t first_igpu = nullptr;

        for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
            auto* dev = ggml_backend_dev_get(i);
            if (!dev) continue;  // Skip null/failed devices (e.g., HTP init failure)

            auto dev_type = ggml_backend_dev_type(dev);

            if (dev_type == GGML_BACKEND_DEVICE_TYPE_GPU && !first_gpu) {
                first_gpu = dev;
            } else if (dev_type == GGML_BACKEND_DEVICE_TYPE_IGPU && !first_igpu) {
                first_igpu = dev;
            }
        }

        // Check the device that will actually be used (GPU has priority over IGPU)
        ggml_backend_dev_t device_to_check = first_gpu ? first_gpu : first_igpu;
        if (device_to_check) {
        }
    }
...
int32_t LlamaLlm::generate(const ml_LlmGenerateInput* input, ml_LlmGenerateOutput* output) {
    ...
    if (!has_input_ids && !has_prompt_utf8)
        return ML_ERROR_COMMON_INVALID_INPUT;  // error: neither input_ids nor prompt_utf8 provided
    ...
    if (has_input_ids) {
        ...
        for (int32_t i = 0; i < input->input_ids_count; i++) {
            if (input->input_ids[i] < 0 || input->input_ids[i] >= vocab_size) {
                ...
                return ML_ERROR_COMMON_INVALID_INPUT;  // error: token ID out of vocabulary range
            }
        }
    ...
    GENIEX_LOG_DEBUG("prefix match: past_prompt_tokens size: {}, prompt_len: {}, match_len: {}",
    ...
    auto process = [&](std::vector<llama_token>& embd) {
        ...
        if (this->n_past + (int)embd.size() >= n_ctx) {
            ...
            GENIEX_LOG_INFO(
                "Context shifting - discarding {} tokens, n_keep: {}, this->n_past before: {}, this->n_past after: "
                "{}",
                n_discard,
                n_keep,
                n_past_before,
        ...
        for (int i = 0; i < (int)embd.size(); i += n_batch) {
            ...
            if (llama_decode(this->ctx, batch) != 0) {
                return ML_ERROR_LLM_GENERATION_FAILED;  // error: llama_decode failed during token processing
            }
    ...
    for (size_t i = 0; i < embd_inp.size(); ++i) {
        common_sampler_accept(this->sampler, embd_inp[i], /* accept_grammar= */ false);
...
void LlamaLlm::set_sampler(const ml_SamplerConfig* cfg) {
    ...
    if (cfg) {
        // Apply sampling parameters from the config, using defaults for 0/0.0 values
        // This matches the pattern from the obsolete ml-llm.cpp implementation
        ...
        // Handle grammar configuration - prioritize grammar_string over grammar_path
        if (cfg->grammar_string && strlen(cfg->grammar_string) > 0) {
            s.grammar = cfg->grammar_string;
    ...
    // Note: if cfg is null, s will use default values from common_params_sampling constructor
