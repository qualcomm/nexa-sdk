...
- **芯片** —— GenieX 面向高通骁龙芯片。详见[支持的平台](/cn/get-started/platforms)，覆盖计算、移动与 IoT 芯片。
- **运行时** —— 选择哪个运行时取决于你的模型。详见[选择运行时](/cn/get-started/choose-runtime)。
- **模型** —— 各运行时经过验证的示例见[支持的模型](/cn/models/supported)。
