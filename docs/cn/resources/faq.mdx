...
<AccordionGroup>
  ...
  <Accordion title="没有实体设备如何测试 Android demo？" id="how-do-i-test-the-android-demo-without-a-physical-device">
    在 QDC 上使用 **骁龙 8 Elite** 或 **8 Elite Gen 5** 的交互式会话，并安装预构建的 demo APK。
...
<AccordionGroup>
  <Accordion title="check.sh 报告缺少库——我该安装什么？">
    预检脚本 (`check.sh`) 与 Docker 入口脚本 (`entrypoint.sh`) 会提示系统库或高通驱动缺失。请在**主机**上运行以下命令——如果当前在 geniex 容器内，请先 `exit`。在 Ubuntu / Debian 上用两条 `apt` 命令安装。
    ...
    **2. 高通驱动包（Adreno GPU + FastRPC NPU bridge）：**
