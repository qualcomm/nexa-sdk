...
本页将带你从 Kotlin 应用中运行第一个模型，然后再换用另一个模型。如需一个完整的参考应用——包含聊天 UI、模型选择器和 VLM 支持——请参阅[示例应用](https://github.com/qcom-ai-hub/geniex/tree/main/examples/android)。
...
切换模型主要就是更改 `model_name` 和 `plugin_id`。这里有两种运行时：

- **`llama_cpp`** —— 运行任意 GGUF 模型。通过 `device_id` 支持 NPU / GPU / CPU 计算单元。
- **`qairt`**（Qualcomm AI Engine Direct）—— 运行 Qualcomm AI Hub Models。仅支持 NPU，在 Android 上需要显式指定 `chipset`。

### 另一个 GGUF 模型 (llama.cpp)

只需更改 `model_name`（如果想要不同精度，再更改 `quant`）——其余流程完全相同：
...
然后在 `LlmCreateInput` 中切换为 `plugin_id = "qairt"`。受支持的 Qualcomm AI Hub 仓库参阅 [API 参考](/cn/run/android/api-reference#已支持模型)。

### 切换计算单元 (NPU / GPU / CPU)

仅适用于 `llama_cpp`——在 `LlmCreateInput` 上设置 `device_id`：
...
位于 `examples/android/` 的[示例应用](https://github.com/qcom-ai-hub/geniex/tree/main/examples/android)是一个完整连通的聊天客户端，构建在上述代码片段之上。当你构建自己的 UI 时，有几个值得借鉴的模式：
