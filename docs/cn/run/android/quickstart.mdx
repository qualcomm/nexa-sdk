...
本页将指导你在 Kotlin APP中运行第一个模型，然后再换用另一个模型。如需一个完整的参考应用——包含聊天 UI、模型选择器和 VLM 支持——请参阅[示例应用](https://github.com/qualcomm/GenieX/tree/main/examples/android)。
...
位于 `examples/android/` 的[示例应用](https://github.com/qualcomm/GenieX/tree/main/examples/android)是一个完整连通的聊天客户端，构建在上述代码片段之上。当你构建自己的 UI 时，有几个值得借鉴的模式：
