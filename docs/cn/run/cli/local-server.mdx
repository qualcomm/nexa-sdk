...
stream = client.chat.completions.create(
    model="unsloth/Qwen3-4B-GGUF:Q4_0",  # org/repo[:precision]，Q4_0 对 Hexagon NPU 支持最佳
    messages=[
        {"role": "user", "content": "Hello! Briefly introduce yourself."},
    ],
    max_tokens=256,
...
<Note>
请将 `model` 替换为已经拉取的模型。可选的 `:<precision>` 后缀用于选择量化版本（例如 `Q4_0`、`Q4_K_M`、`Q8_0`），`Q4_0` 在 Hexagon NPU 上的 llama.cpp 运行时支持最佳。详见[量化](/cn/models/quantization)。
</Note>
