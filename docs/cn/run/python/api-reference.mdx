...
| `device_map` | `str` | `"auto"` | `"auto"` 选择第一个可用的插件/设备。也支持 `"<plugin_id>"` 或 `"<plugin_id>:<device_id>"`。 |
| `quant` | `str \| None` | `None` | 量化版本（例如 `"Q4_K_M"`）。从 Hub 下载时用于过滤文件。 |
...
| 函数 | 说明 |
|---|---|
| `pull(model_name, ...)` | 按别名或 `org/repo[:quant]` 下载模型。 |
...
| `quant` | `str \| None` | `None` | 量化提示（例如 `"Q4_K_M"`）。 |
...
| 函数 | 说明 |
|---|---|
| `geniex.init()` | 初始化运行时。首次加载模型时自动调用。 |
| `geniex.deinit()` | 关闭运行时并释放资源。 |
| `geniex.version()` | 返回 SDK 版本字符串。 |
| `geniex.get_plugin_list()` | 返回可用插件 ID 列表 `list[str]`。 |
| `geniex.get_device_list(plugin_id)` | 返回 `(device_id, device_name)` 元组列表。 |
