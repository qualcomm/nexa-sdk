...
| `device_map` | `str` | `"auto"` | `"auto"` 选择第一个可用的运行时与计算单元。也支持 `"<plugin_id>"`（运行时）或 `"<plugin_id>:<device_id>"`（运行时 + 计算单元）。 |
...
| `geniex.get_plugin_list()` | 返回可用运行时 ID 列表 `list[str]`（即 `plugin_id`）。 |
| `geniex.get_device_list(plugin_id)` | 返回指定运行时下的 `(device_id, device_name)` 计算单元元组列表。 |
