...
Choose the inference runtime via `plugin_id`:

```kotlin
val plugin_id: String?   // "llama_cpp" | "qairt" | null
```

| `plugin_id`   | Runtime                          | Model format             | Compute units                  |
|---------------|----------------------------------|--------------------------|--------------------------------|
...
Runs any GGUF model on CPU, Adreno GPU, or Hexagon NPU. Compute-unit selection is controlled by `device_id`.
