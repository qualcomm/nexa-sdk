...
Swapping models is mostly a matter of changing the `model_name` and the `plugin_id`. There are two runtimes:

- **`llama_cpp`** — runs any GGUF model. Supports NPU / GPU / CPU compute units via `device_id`.
...
- **Runtime-aware compute-unit picker** — when the selected model is QAIRT, the picker hides GPU/CPU options. See `LoadDialog.kt`.
...
<CardGroup cols={2}>
  ...
  <Card title="Runtimes" href="/en/get-started/choose-runtime" icon="route">
