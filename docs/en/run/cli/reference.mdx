...
On first run (for both QAIRT and llama.cpp plugins), pick your device:
...
**2. Quantization (llama.cpp only)**

For GGUF models the CLI prompts you to pick a quantization:
...
<Tip>`Q4_0` has the best Hexagon NPU support. See [Quantization](/en/models/quantization).</Tip>

QAIRT models are pre-quantized — no choice needed.
...
**Device selection** — choose the backend (default: `npu`):
...
<Warning>QAIRT models run on NPU only. Using `--device cpu` or `--device gpu` with a QAIRT model returns an error.</Warning>
