...
<Note>
Replace the `model` value with a model you have already pulled. The optional `:<precision>` suffix selects a quantization variant (e.g. `Q4_0`, `Q4_K_M`, `Q8_0`) — `Q4_0` is recommended for llama.cpp on Hexagon NPU. See [Quantization](/en/models/quantization).
</Note>
