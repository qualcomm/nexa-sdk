...
<Steps>
  ...
  <Step title="Provide the token to GenieX">
    GenieX checks the following sources in order — use whichever is most convenient:
...
<Tip>If both `HF_TOKEN` and `~/.cache/huggingface/token` exist, the environment variable wins.</Tip>
