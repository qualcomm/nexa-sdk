...
| **Linux ARM64** *(Dragonwing IoT — cameras, robotics, industrial)* | <ul><li>Dragonwing QCS9075</li></ul> | <ul><li>[Native install](/en/run/cli/install)</li><li>[Docker](/en/run/linux/install)</li></ul> |
...
The precision you pick at `geniex pull` time also determines where the model lands — see [Precisions (Quantizations) & device placement](/en/models/supported#precisions-quantizations--device-placement).
...
Qualcomm AI Hub pulls on Android **require an explicit chipset** — auto-detect only runs on Windows on Snapdragon. Use:
...
- Adding a new Qualcomm AI Hub Model requires registering it on the C++ side first (`third-party/geniex-qairt/models/{llm,vlm}_model_registry.h`).
