---
title: "Local Server"
description: "OpenAI-compatible local inference server for building applications and agentic workflows"
---

import Feedback from '/snippets/page-feedback.mdx';

## **Overview**

GenieX includes a built-in local inference server that exposes an **OpenAI-compatible API**. This lets you run models on-device and connect them to any application or framework that speaks the OpenAI protocol — including agentic workflows like **LangChain**, AI-native apps like **Open WebUI**, and your own custom applications. No cloud dependency, no API keys — just a local endpoint backed by Qualcomm hardware acceleration.

## **Getting Started**

If you haven't already, pull a model before making requests. For example, pull Qwen3:

```bash bash
geniex pull Qualcomm/Qwen3-4B-Instruct
```

To use the API, first open a terminal from the project root. Then, start the GenieX server by:

```bash bash
geniex serve
```

The server runs on `http://127.0.0.1:18181` by default. \
Keep the terminal that runs the server open, and make your requests from another terminal tab. \
To see a full list of configurable options for the server, run `geniex serve -h`


## **/v1/chat/completions**

Creates a model response for a given conversation. Supports LLM(text-only) and VLM (image+text).

### **Use LLM**

#### **Request body**

```json Example Value
{
  "model": "Qualcomm/Qwen3-4B-Instruct",
  "messages": [
    {"role": "user", "content": "Hello! Briefly introduce yourself."}
  ],
  "max_tokens": 256,
  "temperature": 0.7,
  "stream": false
}
```

#### **Usage Example**

<CodeGroup>
  ```bash Windows (cmd)
  curl -X POST http://127.0.0.1:18181/v1/chat/completions -H "Content-Type: application/json" -d "{\"model\": \"Qualcomm/Qwen3-4B-Instruct\", \"messages\": [{\"role\": \"user\", \"content\": \"Hello!\"}], \"max_tokens\": 64}"
  ```

  ```bash Linux
  curl -X POST http://127.0.0.1:18181/v1/chat/completions \
    -H "Content-Type: application/json" \
    -d '{
      "model": "Qualcomm/Qwen3-4B-Instruct",
      "messages": [{"role":"user","content":"Hello!"}],
      "max_tokens": 64
    }'
  ```
</CodeGroup>

### **Use VLM**

#### **Request body**

`image_url` could one of the following:

- A remote URL, e.g., `https://example.com/photo.jpg`
- A base64-encoded string, e.g., `data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD...`
- A local file path on server machine, which requires the server to have access to the file. e.g., `file:///C:/Users/Username/Pictures/photo.jpg` (Windows) or `file:///home/username/Pictures/photo.jpg` (Linux), `file://` is optional, you can just provide the path like `C:/Users/Username/Pictures/photo.jpg` or `/home/username/Pictures/photo.jpg`.

```json Example Value
{
  "model": "Qualcomm/qwen3vl-GGUF",
  "messages": [
    {
      "role": "user",
      "content": [
        {"type": "text", "text": "Describe this image succinctly."},
        {"type": "image_url", "image_url": {"url": "</path/to/image>"}}
      ]
    }
  ]
}
```

#### **Usage Example**

<CodeGroup>
  ```bash Windows (cmd)
  curl -X POST http://127.0.0.1:18181/v1/chat/completions -H "Content-Type: application/json" -d "{\"model\": \"NexaAI/qwen3vl-GGUF\", \"messages\": [{\"role\": \"user\", \"content\": [{\"type\": \"text\", \"text\": \"what is main color of the picture\"}, {\"type\": \"image_url\", \"image_url\": {\"url\": \"</path/to/image>\"}}]}], \"stream\": false}"
  ```

  ```bash Linux
  curl -X POST http://127.0.0.1:18181/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "NexaAI/qwen3vl-GGUF",
    "messages": [
      {
        "role": "user",
        "content": [
          {
            "type": "text",
            "text": "what is main color of the picture"
          },
          {
            "type": "image_url",
            "image_url": {"url": "</path/to/image>"}
          }
        ]
      }
    ],
    "stream": false
  }'
  ```
</CodeGroup>


<br />

<Feedback/>
