...
Complete API documentation for the GenieX Python SDK — model loading, generation, streaming, tokenization, and model management.

The SDK follows the same design patterns as Hugging Face `transformers` — use `AutoModel*.from_pretrained()` to load a model, then call `.generate()` for inference.
...
model = AutoModelForVision2Seq.from_pretrained("qualcomm/Qwen2.5-7B-Instruct", device_map="qairt")
```
...
|---|---|
| `pull(model_name, ...)` | Download a model by alias or `org/repo[:quant]`. |
...
| Function | Description |
|---|---|
| `geniex.init()` | Initialize the runtime. Called automatically on first model load. |
| `geniex.deinit()` | Shut down the runtime and release resources. |
| `geniex.version()` | Returns the SDK version string. |
| `geniex.get_plugin_list()` | Returns `list[str]` of available plugin IDs. |
| `geniex.get_device_list(plugin_id)` | Returns `list[tuple[str, str]]` of `(device_id, device_name)` pairs. |
